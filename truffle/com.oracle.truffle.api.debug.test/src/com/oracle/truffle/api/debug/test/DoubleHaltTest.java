...
public class DoubleHaltTest extends AbstractDebugTest {
    ...
    public void testCallLoopStepInto() throws Throwable {
        ...
        try (DebuggerSession session = startSession()) {
            ...
            for (int i = 0; i < 3; i++) {
                SuspendedEvent event = checkState(expectSuspended(), 4, true, "STATEMENT");
                assertEquals(1, event.getBreakpoints().size());
                assertSame(breakpoint4, event.getBreakpoints().iterator().next());
                switch (i % 3) {
                    case 0:
                        /*
                         * Note Chumer: breakpoints should always hit independent if we are
                         * currently stepping out or not. thats why step out does not step out here.
                         */
