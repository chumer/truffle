 ...
 *
 * Example usage: {@link com.oracle.truffle.api.debug.DebuggerTesterSnippets#testDebugging()}
 *
 * @since 0.16
 */
public final class DebuggerTester implements AutoCloseable {
    ...
    public DebuggerTester() {
        ...
        this.evalThread = new Thread(new Runnable() {
            public void run() {
     ...
     * event is only valid until on of {@link #expectKilled()}, {@link #expectSuspended()} or
     * {@link #expectDone()} is called again. Throws an {@link IllegalStateException} if the tester
     * is already closed.
     *
     * @since 0.16
     * @return a suspended event for assertions
     */
    public SuspendedEvent expectSuspended() {
        if (closed) {
        ...
        if (event instanceof ExecutingSource) {
            ...
            ((SuspendedEvent) event).thread = Thread.currentThread();
            return (SuspendedEvent) event;
        } else {
    ...
    public void expectKilled() {
        ...
        if (error instanceof KillException) {
            return;
        }
    ...
    private void onSuspend(SuspendedEvent event) {
        ...
        try {
            // hack to materialize stack frames
    ...
    public void testDebugging() {
        try (DebuggerTester tester = new DebuggerTester()) {
            ...
            try (DebuggerSession session = tester.startSession()) {
                ...
                SuspendedEvent event = tester.expectSuspended();
                // assert suspended event is proper here
                event.prepareStepInto(1);

                event = tester.expectSuspended();
