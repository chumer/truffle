...
/**
 * A breakpoint represents a location on the guest language application that should suspend the
 * execution. New breakpoints can be created using a builder based on a {@link #newBuilder(Source)
 * source} a source {@link #newBuilder(URI) URI} or an exact {@link #newBuilder(SourceSection)
 * source section}. New breakpoints are {@link DebuggerSession#install(Breakpoint)} installed for a
 * {@link DebuggerSession debugger session}.
 * <p>
...
public final class Breakpoint {
    ...
    /**
     * Returns <code>true</code> if the this breakpoint has be resolved, in other words the code
     * where this breakpoint is suspended on has been loaded.
     *
     * @since 0.16
     */
    ...
    /*
     * CHumer Deprecation Note: I deprecated it because I wanted to get rid of the IOException in
     * setCondition. A new setConditionText is also fits better to the new getConditionText.
     */
    /**
     ...
     * @since 0.9
     * @deprecated in 0.16: use {@link #setConditionExpression(String)} instead.
     */
    @Deprecated
    public synchronized void setCondition(String expr) throws IOException {
        setConditionExpression(expr);
    }

    /*
     * CHumer Deprecation Note: I was going back and forth on deprecating this or not. This methods
     * semantics are very complicated and the mime type of a breakpoint can only be returned
     * reliably if it was resolved (if the breakpoint is based on URIs). To me it was very
     * surprising that the return type of the getCondition() is Source instead of String and that it
     * might be null.
     */
    /**
     ...
     *
     * @since 0.9
     * @see #getConditionExpression() to get the unresolved condition text.
     * @deprecated in 0.16, unreliable implementation. use {@link #getConditionExpression()}
     *             instead.
     */
    @Deprecated
     ...
     *
     * @param condition if non{@code -null}, a boolean expression, expressed in the guest language,
     *            to be evaluated in the lexical context at the breakpoint location.
     * @throws UnsupportedOperationException if the breakpoint does not support conditions
     * @since 0.16
     */
    public synchronized void setConditionExpression(String condition) {
    ...
    /**
     * Returns the expression that defines the current condition on this breakpoint; {@code null} if
     * this breakpoint is currently unconditional.
     *
     * @since 0.16
     */
    public synchronized String getConditionExpression() {
        return condition;
    }
    ...
    public static final class Builder {
        ...
        public Breakpoint build() {
            SourceSectionFilter filter = buildFilter();
            BreakpointLocation location = new BreakpointLocation(key, line, -1);
            Breakpoint breakpoint = new Breakpoint(location, filter, oneShot);
        ...
        private SourceSectionFilter buildFilter() {
            ...
            if (key instanceof URI) {
                ...
                filter.sourceIs((Source) key);
            }
            if (line != -1) {
                filter.lineStartsIn(IndexRange.byLength(line, 1));
            }
            if (sourceSection != null) {
                filter.sourceSectionEquals(sourceSection);
            }
            filter.tagIs(StatementTag.class);
            return filter.build();
        }
    }
    ...
    private static class ConditionalBreakNode extends Node {
        ...
        private void initializeConditional() throws IOException {
            ...
            synchronized (breakpoint) {
                conditionSource = Source.newBuilder(breakpoint.getConditionExpression()).mimeType(context.getInstrumentedSourceSection().getSource().getMimeType()).name(
