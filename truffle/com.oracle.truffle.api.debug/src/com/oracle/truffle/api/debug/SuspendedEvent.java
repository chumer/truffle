...
/**
 * <p>
 * A suspended event is delivered to a {@link Debugger debugger} {@link DebuggerSession session}
 * whenever the execution was suspended by a {@link Breakpoint breakpoint} or stepping. The event is
 * delivered using a {@link SuspendedCallback} implementation that is provided when a new debugger
 * session is {@link Debugger#startSession(SuspendedCallback) started}. An exclusive suspended event
 * instance is delivered for each thread. The suspended event must only be used from the thread it
 * was created for and while {@link SuspendedCallback#onSuspend(SuspendedEvent)} is currently
 * executing otherwise an {@link IllegalStateException} is thrown by all methods of this class. In
 * other words a suspended event becomes invalid when the callback finished execution.
 * </p>
 * <p>
 * The next stepping behavior can be configured by preparing a stepping action:
 * <ul>
 * <li>{@link #prepareStepInto(int) Step into}</li>
 * <li>{@link #prepareStepOut() Step out}</li>
 * <li>{@link #prepareStepOver(int) Step over}</li>
 * <li>{@link #prepareKill() Kill}</li>
 * <li>{@link #prepareContinue() Continue}</li>
 * </ul>
 * If no stepping action is prepared then the stepping action {@link #prepareContinue() continue} is
 * assumed. Only a single action can be prepared otherwise an {@link IllegalStateException} is
 * thrown.
 * </p>
 * <p>
 * Other meta data that can be queried using a suspended event:
 * <ul>
 * <li>{@link #getStackFrames() Current stack frames} indicating the current guest language
 * application location.</li>
 * <li>{@link #getReturnValue() Return value} when the execution is halted {@link #isHaltedBefore()
 * after} a frame.</li>
 * </ul>
 * </p>
 *
